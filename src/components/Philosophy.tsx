...
import { Scan, Dumbbell, Trophy, ArrowRight } from 'lucide-react'
      ...
      <div className="w-full max-w-[1400px] mx-auto px-6 md:px-10 lg:px-16">
        ...
        <div ref={timelineRef} className="relative mb-24">
          ...
          <div
            ...
            style={{
              width: activeStep === null ? '0%' : `${(activeStep / (journeySteps.length - 1)) * 100}%`,
            }}
          />
          ...
          <div className="grid md:grid-cols-3 relative">
            {journeySteps.map((step, i) => (
              ...
              >
                ...
                >
                  <step.icon
                    ...
                    className={`transition-colors duration-300 ${
                      activeStep === i ? 'text-white' : 'text-accent'
                    }`}
                    strokeWidth={1.5}
                  />
                </div>
                ...
                <p className={`mt-5 text-[13px] font-bold uppercase tracking-[0.15em] transition-colors duration-300 ${
                  activeStep === i ? 'text-accent' : 'text-text-tertiary'
                }`}>
                  Step {step.step}
                </p>
                <h3 className={`mt-2 text-[20px] font-heading font-bold transition-colors duration-300 ${
                  activeStep === i ? 'text-text-primary' : 'text-text-primary/70'
                }`}>
                  {step.title}
                </h3>
                {step.optional && (
                  <p className="text-[12px] text-text-tertiary mt-1 italic">(optional)</p>
                )}
          ...
          {/* Mobile fallback: show all expanded on small screens */}
          <div className="md:hidden mt-8 space-y-4">
            {journeySteps.map((step) => (
              <div key={step.step} className="bg-white rounded-2xl border border-warm-border p-6 shadow-[0_4px_16px_rgba(0,0,0,0.04)]">
                <div className="flex items-center gap-3 mb-3">
                  <div className="w-10 h-10 rounded-full bg-accent/10 flex items-center justify-center">
                    <step.icon size={18} className="text-accent" strokeWidth={1.5} />
                  </div>
                  <div>
                    <h4 className="text-[18px] font-heading font-bold">{step.title}</h4>
                    <p className="text-[13px] font-semibold text-accent">{step.subtitle}</p>
                  </div>
                </div>
                <p className="text-[14px] text-text-secondary leading-[1.7] mb-3">{step.description}</p>
                <span className="text-[11px] font-bold uppercase tracking-[0.15em] text-accent/60">{step.cta}</span>
              </div>
            ))}
          </div>
        </div>
